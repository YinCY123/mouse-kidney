...
cell_barcode_ids <- read.csv("../data/kidney-map/GSE175421/GSE175421_cell-barcode-ids.csv.gz") %>% 
    clean_names()
...
files <- dir_ls("../data/kidney-map/GSE175421/", regexp = "*.txt.gz$")
...
saveRDS(drg_sce, "../data/kidney-map/GSE175421/drg_sce.rds")
```


```{r}
drg_sce <- readRDS("../data/kidney-map/GSE175421/drg_sce.rds")
...
source("R/idconv.R")
plotExpression(drg_sce, features = idconv("Th"), 
               x = "main_cluster")
...
run_info <- read.table("../data/kidney-map/GSE78845/SraRunTable.txt", sep = ",", header = TRUE) %>% clean_names()
rownames(run_info) <- run_info$run

feature_counts <- readRDS("../data/kidney-map/GSE78845/feature_counts.rds")
...
rd <- counts(sym_sce) %>% rowSums() > 0
cd <- counts(sym_sce) %>% colSums() > 100
sym_sce <- sym_sce[rd, cd]
sym_sce
saveRDS(sym_sce, "../data/kidney-map/GSE78845/sym_sce.rds")
```
...
is.mito <- which(rowData(sym_sce)$chr == "MT")
is.mito %>% table
...
sym_sce <- runUMAP(sym_sce, dimred = "PCA")
