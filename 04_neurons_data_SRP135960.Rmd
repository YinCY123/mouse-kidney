...
lib_size <- pooledSizeFactors(counts(drg_sce), clusters = drg_sce$main_cluster)
...
plotExpression(drg_sce, features = idconv("Calca"), 
               x = "cell_type")

saveRDS(drg_sce, "../data/kidney/kidney-map/GSE175421/drg_sce.rds")
colData(drg_sce) <- transform(colData(drg_sce), 
                              mito_percent = percent_mt)
