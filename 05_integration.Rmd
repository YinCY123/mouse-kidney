...
library(scater)
library(patchwork)
harmony_integrated <- readRDS("../data/kidney/kidney-map/integrated_data/harmony_integrated.rds")

plotReducedDim(harmony_integrated, 
               dimred = "TSNE", 
               colour_by = "cell_type", 
               text_by = "cell_type", 
               text_size = 2,
...
plotReducedDim(harmony_integrated, 
               ...
               text_size = 2,
               point_size = 0.2) +
    scale_color_discrete(name = "region", labels = c("Cortex", "inner medulla", "outer medulla", "paravertebral")) +
    guides(color = guide_legend(override.aes = list(size = 2))) +
    theme(legend.position = "right") + 
    plot_layout(ncol = 1)

ggsave("../data/kidney/kidney-map/results/integration_maps/harmony_integrated.pdf", 
       height = 15, width = 7)
```
...
RISC_integrated <- readRDS("../data/kidney/kidney-map/integrated_data/RISC_integrated.rds")

RISC_integrated_df <- left_join(RISC_integrated@coldata, 
                                RISC_integrated@DimReduction$cell.tsne %>% as.data.frame() %>% tibble::rownames_to_column("Barcode"))
RISC_integrated_df <- transform(RISC_integrated_df, 
                                Set = sapply(RISC_integrated_df$Set, function(x){
...
cell_location <- RISC_integrated_df %>% 
    group_by(cell_type) %>% 
    summarise(x = median(tSNE1), 
              y = median(tSNE2))

p1 <- RISC_integrated_df %>% 
    ggplot(aes(tSNE1, tSNE2)) +
    geom_point(aes(color = cell_type), size = 0.2) +
    geom_text(data = cell_location, aes(x, y, label = cell_type), size = 2) +
    theme_classic() +
    theme(legend.position = "none")

p2 <- RISC_integrated_df %>% 
    ggplot(aes(tSNE1, tSNE2)) +
...
ggsave("../data/kidney/kidney-map/results/integration_maps/RISC_integrated.pdf", 
       height = 15, width = 7)
```
...
library(patchwork)
library(scater)
fastMM_integrated <- readRDS("../data/kidney/kidney-map/integrated_data/fastMNN_integrated.rds")

plotReducedDim(fastMM_integrated, 
               dimred = "TSNE", 
               colour_by = "cell_type", 
               text_by = "cell_type", 
               text_size = 2, 
...
plotReducedDim(fastMM_integrated, 
               ...
               text_size = 2, 
               point_size = 0.2) +
    scale_color_discrete(name = "region", labels = c("Cortex", "outer medulla", "inner medulla", "paravertebral")) +
    theme(legend.position = "right") +
    guides(color = guide_legend(override.aes = list(size = 2))) +
    plot_layout(ncol = 1)

ggsave("../data/kidney/kidney-map/results/integration_maps/fastMNN_integrated.pdf", 
       height = 15, width = 7)
```
...
Seurat_integrated <- readRDS("../data/kidney/kidney-map/integrated_data/Seurat_integrated.rds")
cols <- colorRampPalette(colors = brewer.pal(9, "Set1"))(length(unique(Seurat_integrated[[]]$cell_type)))
names(cols) <- Seurat_integrated[[]]$cell_type %>% unique
...
Seurat_integrated[[]] %>% table(useNA = "ifany")
```
