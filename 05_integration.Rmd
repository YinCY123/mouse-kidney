...
cs <- combined_sces[, combined_sces$clusters %in% c(1, 7, 43, 52)]
...
colLabels(cs) <- paste("1_52.", colLabels(cs), sep = "") 

labels <- as.character(combined_sces$clusters)
labels[labels %in% c(1, 7, 43, 52)] <- colLabels(cs)
combined_sces$clusters <- labels
...
```{r, message=FALSE}
# CCD_PC, OMCD_PC, IMCD_PC: 12, 15, 36, 65
cs <- combined_sces[, combined_sces$clusters %in% c(12, 15, 36, 65)]
...
colLabels(cs) <- paste("12_65.",colLabels(cs), sep = "") 

labels <- as.character(combined_sces$clusters)
labels[labels %in% c(12, 15, 36, 65)] <- colLabels(cs)
combined_sces$clusters <- labels
