...
cell_location <- colData(combined_sces) %>% as.data.frame() %>% cbind(reducedDim(combined_sces, 2)) %>% 
    group_by(cell_type) %>% 
...
ggcells(combined_sces, aes(TSNE.1, TSNE.2)) +
    geom_point(aes(colour = mito_percent), size = 0.5, alpha = 1/2, show.legend = T) +
    geom_text_repel(data = cell_location, aes(x, y, label = cell_type), size = 2.5) +
    viridis::scale_color_viridis(name = "mitochondrial \npercentage", 
                                 discrete = F, 
                                 breaks = c(0, 25, 50, 75),
...
ggsave("results/figures/fastMNN_mito_percent.pdf")
```
