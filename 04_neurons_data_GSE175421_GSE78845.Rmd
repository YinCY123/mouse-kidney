...
drg_cell_barcode_ids$cell_type <- ifelse(drg_cell_barcode_ids$cell_type == "Vascular Endothelial Cells", "paravertebral Endothelial", drg_cell_barcode_ids$cell_type)
drg_cell_barcode_ids$location <- "paravertebral"
...
colData(drg_sce)$cell_type <- sapply(colData(drg_sce)$clusters, 
                                        FUN = function(x){
                                            switch(as.character(x), 
                                                   ...
                                                   "6" = "Schwann Cells", 
                                                   "7" = "Schwann Cells", 
                                                   "8" = "paravertebral Endothelial", 
                                                   "9" = "Sensory Neurons", 
                                                   "10" = "paravertebral Endothelial")
                                        })
colData(drg_sce)$cell_type %>% table
source("functions/idconv.R")
plotTSNE(drg_sce, 
         # colour_by = "cell_type",
         colour_by = idconv("Tubb3"),
         point_size = 0.5, 
         text_by = "clusters")
...
run_info <- read.table("data/GSE78845_SraRunTable.txt", sep = ",", header = TRUE) %>% 
    clean_names()
selected_vars <- c("run", "cell_type", "celltype_identifier", "source_name", "age")
run_info <- run_info %>% dplyr::select(selected_vars) %>% 
    dplyr::rename(cell_ids = "run")
run_info$cell_type <- ifelse(run_info$cell_type == "sympathetic neuronal cells", "Sympathetic Neurons", run_info$cell_type)
run_info$location <- "paravertebral"
...
ids <- match(rownames(run_info), colnames(mtx))
mtx <- mtx[, ids]
