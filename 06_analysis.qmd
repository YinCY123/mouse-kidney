...
level3_markers <- list(
    ...
    "Afferent/Efferent Endothelial" = c(""),
    "Glomerular Endothelial" = c("Grasp", "Pi16", "Gm49393", "Pde2a", "Gm49391"),
    "Spinal Endothelial" = c(""), 
    "Parietal Epithelial" = c("Stc2", "C3"), 
    "PTS1" = c("Slc34a1"), 
    "PTS2" = ,
    "PTS3" = ,
...
source("functions/idconv.R")
idconv("ENSMUSG00000024011", from_type = "GENEID", to_type = "SYMBOL", db = "mouse")
...
p0 <- readRDS("../data/kidney/kidney-map/results/p_cluster_relationships.rds")
p1 <- readRDS("../data/kidney/kidney-map/results/markers/p_level1_markers.rds")
p2 <- readRDS("../data/kidney/kidney-map/results/markers/p_level2_markers.rds")
p3 <- readRDS("../data/kidney/kidney-map/results/markers/p_level3_markers.rds")

p0 + ((p1 + p2 + plot_layout(widths = c(1, 2.3))) / p3 + plot_layout(heights = c(4, 5))) + plot_layout(widths = c(1, 5),ncol = 2)
    

ggsave("../data/kidney/kidney-map/results/markers/marker.pdf", 
       height = 25, width = 40)
```
...
labels <- data.frame(label = c("SORL1" = "APP ",
 ...
 neurons_top_ligands_list[[1]] %>% 
    ...
    geom_text(data = labels, aes(x = 0, y = 1:15, label = label), hjust = 1, size = 3, color = "grey20") +
    geom_text(aes(x = 0, y = 16, label = "ligands"), hjust = 1) +
    geom_text(aes(x = 40, y = 16, label = "receptors"), hjust = 0) +
    coord_cartesian(clip = "off") +
    scale_x_discrete(name = NULL, expand = c(0,0), position = "top") +
    scale_y_discrete(name = NULL, position = "right") +
    scale_size_area(name = "receptor expression level", breaks = c(0.1, 0.5, 1), max_size = 13) +
    theme(axis.text.x = element_text(size = 8, angle = -50, hjust = 1, vjust = 0.5, color = "grey20"), 
          axis.ticks = element_blank(), 
          plot.margin = unit(c(6, 5, 1, 25), units = "mm"),  
          panel.background = element_rect(fill = "white"),
          panel.grid.major = element_line(linetype = 2, linewidth = 0.3, color = "grey70"),
          legend.position = "bottom") 

ggsave("../data/kidney/kidney-map/results/cell_cell_communication/sensory_neuron_top_ligands_receptors_expression.pdf", 
       width = 15, height = 7)


# sympathetic neurons
neurons_top_ligands_list[[2]] %>% dplyr::select(partner_a_receptor, partner_b_ligand) %>% distinct()
labels = c("SORL1" = "APP", 
           "TNFRSF21" = "APP", 
           "TREM2_receptor"  ="APP", 
           "CD74" = "APP", 
          ...
          "DRD1" = "SerotoninDopamin_byDDC\nDopamine_byDDC_and_SLC18A1\nDopamine_byDDC_and_SLC18A2", 
          "DRD2" = "SerotoninDopamin_byDDC\nDopamine_byDDC_and_SLC18A1\nDopamine_byDDC_and_SLC18A2", 
          "DRD3" = "SerotoninDopamin_byDDC\nDopamine_byDDC_and_SLC18A1\nDopamine_byDDC_and_SLC18A2", 
...
neurons_top_ligands_list[[2]] %>% 
    ...
    geom_point(aes(size = log1p(gene_a_receptor_expression))) +
    geom_text(data = labels, aes(x = 0, y = 1:38, label = label), hjust = 1, size = 2.5, color = "grey20") +
    geom_text(aes(x = 0, y = 39, label = "ligands"), hjust = 1, size = 5) +
    geom_text(aes(x = 40, y = 39, label = "receptors"), hjust = 0, size = 5) +
    coord_cartesian(clip = "off") +
    scale_x_discrete(name = NULL, expand = c(0,0), position = "top") +
    scale_y_discrete(name = NULL, position = "right") +
    scale_size_area(name = "receptor expression level", breaks = c(0.1, 0.5, 1), max_size = 15) +
    theme(axis.text.x = element_text(size = 10, angle = -50, hjust = 1, vjust = 0.5, color = "grey20"), 
          axis.text.y = element_text(size = 13),
          axis.ticks = element_blank(), 
          plot.margin = unit(c(6, 5, 1, 45), units = "mm"),  
          panel.background = element_rect(fill = "white"),
          panel.grid.major = element_line(linetype = 2, linewidth = 0.3, color = "grey70"),
          legend.position = "bottom") 

ggsave("../data/kidney/kidney-map/results/cell_cell_communication/sympathetic_top_ligands_receptors_expression.pdf", 
       width = 25, height = 20)
```
...
saveRDS(neurons_receptors, "../data/kidney/kidney-map/results/cell_cell_communication/")
```
...
labels <- neurons_receptors_top_list[[1]] %>% dplyr::select( partner_b_ligand, partner_a_receptor) %>% distinct() %>% tibble::column_to_rownames("partner_b_ligand")
labels$partner_a_receptor <- paste("  ", labels$partner_a_receptor)
...
neurons_receptors_top_list[[1]] %>% 
    ...
    geom_text(data = labels, aes(x = 39.5, y = 1:43, label = partner_a_receptor), hjust = 0, size = 2, color = "grey20") +
    scale_x_discrete(name = NULL, position = "top") +
    scale_y_discrete(name = NULL) +
    geom_text(x = -1.15, y = 45, label = "ligand") +
    geom_text(x = 41.27, y = 45, label = "neuron\nreceptor") +
    coord_cartesian(clip = "off") +
    scale_size_area(name = "ligands expression level", max_size = 6) +
    theme(axis.text.x = element_text(angle = -60, hjust = 1, size = 6, color = "grey20"), 
          axis.text.y = element_text(size = 6, color = "grey20"),
          panel.background = element_rect(fill = "white"), 
          axis.ticks = element_blank(), 
          panel.grid.major = element_line(linetype = 2, linewidth = 0.1, color = "grey70"), 
          legend.position = "bottom", 
          plot.margin = unit(c(0, 20, 0, 2), "mm"))

ggsave("../data/kidney/kidney-map/results/cell_cell_communication/sensory_top_ligand_receptors_expression.pdf", width = 10)


labels <- data.frame(label = c("ARGN" = "NCAM1", 
                               "REN" = "ATP6AP2", 
                               "EFNA1" = "EPHA5",
                               "EFNA2" = "EPHA5",
                               "EFNA3" = "EPHA5", 
                               ...
                               "BDNF" = "NGFR", 
                               "RLN3" = "RXFP1", 
                               "Acetylcholine_byCHAT_and_SLC10A4" = "AcetylcholineReceptor_a3b4", 
                               "Acetylcholine_byCHAT_and_SLC18A3" = "AcetylcholineReceptor_a3b4", 
                               "Acetylcholine_byCHAT_and_SLC5A7" = "AcetylcholineReceptor_a3b4", 
                               "SCT" = "SCTR", 
                               "VIP" = "SCTR", 
                               ...
                               "GABA_byGAD1_and_SLC32A1" = "GABBR1", 
                               "GABA_byGAD1_and_SLC6A1" = "GABBR1", 
                               "GABA_byGAD1_and_SLC6A11" = "GABBR1"))
labels$label <- paste(" ", labels$label, sep = "")


neurons_receptors_top_list[[2]] %>% 
    ggplot(aes(factor(cell_b_ligand, levels), partner_b_ligand)) +
    geom_point(aes(size = log1p(gene_b_ligand_expression))) +
    geom_text(data = labels, aes(x = 39.5, y = 1:31, label = label), hjust = 0, size = 2, color = "grey20") +
    scale_x_discrete(name = NULL, position = "top") +
    scale_y_discrete(name = NULL) +
    geom_text(x = -1.15, y = 32, label = "ligand") +
    geom_text(x = 41.27, y = 32, label = "receptor") +
    coord_cartesian(clip = "off") +
    scale_size_area(name = "ligands expression level", max_size = 6, breaks = c(0.1, 0.5, 1.0)) +
    theme(axis.text.x = element_text(angle = -60, hjust = 1, size = 8, color = "grey20"), 
          axis.text.y = element_text(size = 6, color = "grey20"),
          panel.background = element_rect(fill = "white"), 
          axis.ticks = element_blank(), 
          panel.grid.major = element_line(linetype = 2, linewidth = 0.1, color = "grey70"), 
          legend.position = "bottom", 
          plot.margin = unit(c(0, 30, 0, 2), "mm"))

ggsave("../data/kidney/kidney-map/results/cell_cell_communication/sympathetic_top_ligand_receptors_expression.pdf",
       width = 10)
```
...
# remove all secreted interactions
means_long %>% dplyr::filter((receptor_a == "False" & receptor_b == "False") & secreted == "False") %>% 
    dplyr::filter(cell_a %in% c("Sensory Neurons", "Sympathetic Neurons")) %>% pull(interacting_pair) %>% unique
```

```{r}
complex_input["integrin_a2b1_complex", ]
```
