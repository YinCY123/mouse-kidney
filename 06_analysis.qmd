...
# select receptors expressed by neurons and ligands expressed by non-neurons
...
receptors_expressed_by_non_neurons_ids <- ((means_long$receptor_a == "False" & means_long$cell_a %in% c("Sympathetic Neurons", "Sensory Neurons")) | (means_long$receptor_b == "False" & means_long$cell_b %in% c("Sympathetic Neurons", "Sensory Neurons")) & (means_long$receptor_a == "True" | means_long$partner_b == "True"))

receptors_expressed_by_neurons_ids %>% table; receptors_expressed_by_non_neurons_ids %>% table
...
neurons_receptors <- receptors_expressed_by_neurons %>% 
    ...
    mutate(gene_a_receptor = ifelse(grepl("^complex:", partner_a_receptor), 
                                    complex_input[gsub("^complex:", "", partner_a_receptor), 1:5, drop = F] %>% 
                                        ...
                                        str_replace(",{1,}$", "") %>% 
                                        str_split(",") %>% 
                                        sapply(function(x){idconv(x, from_type = "UNIPROTID", to_type = "GENEID", db = "human")}) %>% 
                                        str_c(collapse = ","), 
                                    gene_a_receptor)) 
...
receptors_expressed_by_non_neurons <- means_long[receptors_expressed_by_non_neurons_ids, ]
# place receptors into the first column and ligands to the second column
receptors_expressed_by_non_neurons <- receptors_expressed_by_non_neurons %>% 
    rowwise() %>% 
    mutate(cell_a_receptor = ifelse(receptor_a == "True", cell_a, cell_b), 
           partner_a_receptor = ifelse(receptor_a == "True", partner_a, partner_b), 
           gene_a_receptor = ifelse(receptor_a == "True", gene_a, gene_b),
...
neurons_ligands <- receptors_expressed_by_non_neurons %>% 
    rowwise() %>% 
    mutate(gene_b_ligand = ifelse(grepl("^complex:", partner_b_ligand), 
                                  complex_input[gsub("^complex:", "", partner_b_ligand), 1:5, drop = F] %>% apply(1, str_c, collapse = ",") %>% str_replace(",{1,}$", ""), 
                                  gene_b_ligand))

neurons_ligands <- neurons_ligands %>% rowwise() %>% 
    mutate(gene_b_ligand = ifelse(grepl("^ENS", gene_b_ligand), 
                                  gene_b_ligand, 
...
# remove self-interactions
receptors_expressed_by_non_neurons <- receptors_expressed_by_non_neurons %>% dplyr::filter(cell_a_receptor != cell_b_ligand)
...
write.csv(receptors_expressed_by_non_neurons, 
...
# the neurons expressed ligands include organic moleculars, it concentration is proxyed by it's enzyme's expressed level.
# if it's a complex the expressed level is multiple gene's mean expression.
neurons_expressed_ligands <- readRDS("../data/kidney/kidney-map/results/cell_cell_communication/neurons_ligands.rds")
neurons_expressed_ligands <- neurons_expressed_ligands %>% 
    unite(col = "cell_pair_ids", cell_b_ligand, partner_b_ligand, cell_a_receptor, partner_a_receptor, sep = "@", remove = F) 

neurons_expressed_ligands %>% dplyr::filter(is.na(gene_b_ligand))

# convert CEL symbol to ENSG00000170835 geneid
neurons_expressed_ligands$gene_b_ligand <- ifelse(is.na(neurons_expressed_ligands$gene_b_ligand), 
                                                  "ENSG00000170835", neurons_expressed_ligands$gene_b_ligand)

ligand_list <- split(neurons_expressed_ligands$gene_b_ligand, neurons_expressed_ligands$cell_pair_ids) %>% 
    sapply(function(x){str_split(x, ",")})

source("functions/idconv.R")
ligand_list <- ligand_list %>% sapply(function(x){idconv(x, from_type = "GENEID", to_type = "SYMBOL", db = "human")})

# check if there any unsuccessful conversion
ligand_list %>% sapply(function(x){all(is.na(x))}) %>% table
# ligand_list <- ligand_list %>% lapply(function(x){na.omit(x)})

saveRDS(ligand_list, "../data/kidney/kidney-map/results/cell_cell_communication/neuron_expressed_lisgand_list.rds")
ligand_list <- readRDS("../data/kidney/kidney-map/results/cell_cell_communication/neuron_expressed_lisgand_list.rds")

ligand_expression_list <- vector(mode = "list", length = length(ligand_list))
names(ligand_expression_list) <- names(ligand_list)

for(name in names(ligand_expression_list)){
    try(ligand_expression_list[[name]] <- makePerCellDF(sces_human[, sces_human$level3_cluster == str_extract(name, "Sympathetic Neurons|Sensory Neurons")], 
                                                        features = ligand_list[[name]]) %>% 
...
ids <- sapply(ligand_expression_list, length) == 0
ids %>% table
ligand_expression_list[ids] <- NULL
neurons_ligand_expression_df <- ligand_expression_list %>% unlist() %>% 
    as.data.frame() %>% tibble::rownames_to_column() %>% magrittr::set_colnames(value = c("cell_pair_ids", "expression"))
left_join(neurons_ligand_expression_df, neurons_expressed_ligands, by = "cell_pair_ids") %>% 
    arrange(-expression) %>% 
    select(partner_b_ligand, partner_a_receptor, ex)

saveRDS(ligand_expression_list, "../data/kidney/kidney-map/results/cell_cell_communication/ligand_expression_list.rds")
```

```{r}
neurons_expressed_ligands
```


## neurons expressed receptors
```{r}
neurons_expressed_receptors <- read.csv("../data/kidney/kidney-map/results/cell_cell_communication/neurons_receptors.rds")
