...
neurons_top_ligands_list[[1]][, c("partner_a_receptor", "gene_b_ligand")] %>% distinct()
...
ggsave("ligands.pdf", width = 13, height = 7)
```

```{r}
neurons_top_ligands_list[[1]]$partner_a_receptor %>% levels
```
...
neurons_top_receptors_ligands <- split(neurons_receptors, f = neurons_receptors$cell_a_receptor)

neurons_top_receptors_ligands %>% lapply(function(x){
    transform(x, 
              receptor_expression_order = order(gene_a_expression_order, decreasing = TRUE))
})
```
