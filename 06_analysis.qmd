...
fmarker3 %>% names
fmarker3[[4]] %>% rownames() %>% head(50) %>% idconv(from_type = "GENEID", to_type = "SYMBOL", db = "mouse")
...
for(name in names(fmarker3)){
    ...
    ggsave(paste("level3_clusters/", name, "_cluster.pdf", sep = ""), height = 100, width = 10, limitsize = F)
}
...
level2_markers <- list(
    ...
    "Mesangial Cells" = c("Sfrp2", "Dkk2", "Septin4",  "Ccn2", "Adamts5", "F2r", "Itga8", 
                          "Ier5", "Nt5e", "Gdf10", "Scarf2", "Scn1b"),
...
grid.col <- colorRampPalette(colors = rainbow(9))(length(levels(sces$level1_cluster)) - 1)
